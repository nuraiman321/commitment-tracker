...
import {
  monthsLeft, totalLeft, fmt, endDate, errMsg,
          ...
          <tbody>
            {Array.from({ length: 12 }, (_, m) => {
              ...
              const total = commitments.reduce(
                (s: number, c: Commitment) =>
                  monthsLeft(c) - m > 0 ? s + c.amount : s,
                0
              );
              const left = salary - total;
              return (
                <tr key={m} style={{ borderBottom: "1px solid #111" }}>
                  <td style={{ padding: "7px 8px 7px 0", color: "#777" }}>
                    {label}
                  </td>
                  <td
                    style={{
                      textAlign: "right", padding: "7px 8px", color: "#e76f51",
                    }}
                  >
                    {fmt(total)}
                  </td>
                  <td
                    style={{
                      textAlign: "right", padding: "7px 0 7px 8px",
                      color: left < 0 ? "#e76f51" : "#2a9d8f",
                    }}
                  >
                    {fmt(left)}
                  </td>
...
export default function CommitmentPage({ api, onBack }: CommitmentPageProps) {
  ...
  const load = async () => {
    setDataLoad(true);
    try {
      const [me, cmtRes] = await Promise.all([
        api.me(),
        api.getCommitments(),
      ]);
      setUserId(me.data.id);
      setCommitments((cmtRes.data || []) as Commitment[]);

      const profRes = await api.getProfile(me.data.id);
      const prof = profRes.data?.[0];
      if (prof) {
        setSalary(prof.salary || 0);
  ...
  const syncSalary = useCallback(
    ...
    [api, userId, profileId]
  );

  const debouncedSync = useDebounce(syncSalary, 900);

  const onSalaryChange = (val: number) => {
    setSalary(val);
    debouncedSync(val);
  };
  ...
  const handleSave = async (form: CommitmentForm) => {
    ...
    try {
      ...
      if (form.id) {
        ...
        setCommitments((cs) =>
          cs.map((c) => (c.id === form.id ? (res.data as Commitment) : c))
        );
        notify("Updated ✓");
      } else {
  ...
  const pct = salary > 0 ? Math.round((totalCommit / salary) * 100) : 0;
  ...
  const summaryStats: Array<[string, string, string]> = [
    ...
    [
      "Health",
      pct <= 50 ? "✓ Healthy" : pct <= 70 ? "⚠ Watch" : "✗ Critical",
  ...
  return (
    <div
      style={{ minHeight: "100vh", background: "#0a0a0a", fontFamily: sans, color: "#f0ede8" }}
    >
      {/* Header */}
      <div
        style={{
      ...
      >
        ...
        <div>
          <p
            style={{
              fontSize: 9, letterSpacing: 4, color: "#333",
              textTransform: "uppercase", fontFamily: mono,
            }}
          >
            Financial Tracker
          </p>
          <h1 style={{ fontSize: 16, fontWeight: 600, letterSpacing: -0.4 }}>
            Commitment
          </h1>
        </div>
      </div>
      ...
      {dataLoad ? (
        ...
        <div style={{ maxWidth: 560, margin: "0 auto", padding: "24px 20px 80px" }}>
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                >
                  <span style={{ fontSize: 11, color: "#333", fontFamily: mono }}>
                    RM
                  </span>
                  <input
                    type="number"
                    ...
                    style={{
                      ...
                      fontSize: 18, fontFamily: mono, width: "100%",
                ...
                <p
                  style={{
                    fontSize: 9, color: "#2a2a2a", fontFamily: mono, marginTop: 3,
                  }}
                >
                  ...
                  <div key={k}>
                    <p
                      style={{
                        ...
                        textTransform: "uppercase", marginBottom: 2, fontFamily: mono,
                      }}
                    >
                      {k}
                    </p>
          ...
          >
            ...
            <button onClick={() => setModal("add")} style={btn("#e76f51", "#fff")}>
              + Add
            </button>
          </div>
          ...
          {commitments.length === 0 && (
            <div
              style={{
                ...
                color: "#333", fontFamily: mono, fontSize: 12,
              }}
            >
              No commitments yet.
              <br />
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    borderLeft: `3px solid ${c.color || "#e76f51"}`,
                    borderRadius: 10, padding: "14px 16px",
                    opacity: isDel ? 0.4 : 1, transition: "opacity .2s",
                ...
                >
                  ...
                  >
                    <div>
                      ...
                      <p style={{ fontSize: 10, color: "#444", fontFamily: mono }}>
                    ...
                    <div style={{ textAlign: "right" }}>
                      <p
                        style={{
                          ...
                          fontWeight: 500, color: c.color || "#e76f51",
                        }}
                      >
                        {fmt(c.amount)}
                      </p>
                      <p style={{ fontSize: 9, color: "#333", fontFamily: mono }}>
                        /mo
                      </p>
                    </div>
                  </div>
                  ...
                  <ProgressBar
                    paid={c.paid}
                    months={c.months}
                    color={c.color || "#e76f51"}
                  />

                  <div
                    style={{
                      display: "flex", justifyContent: "space-between",
                      marginTop: 7, fontSize: 10, fontFamily: mono, color: "#333",
                    }}
                  >
                    <span>{c.paid}/{c.months} mo · {paidPct}% done</span>
                    <span>Balance {fmt(totalLeft(c))}</span>
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                    <button
                      onClick={() => setModal(c)}
                      disabled={isDel}
                      style={{ ...btn("#141414", "#777"), fontSize: 10, padding: "4px 12px" }}
                    >
                      Edit
                    </button>
                    <button
            ...
            <>
              <button
                ...
                style={{
                  ...
                  border: "1px solid #141414", borderRadius: 8, fontSize: 11,
                }}
              >
                {showProj ? "▲ Hide" : "▼ Show"} 12-Month Projection
              </button>
              {showProj && (
                <Projection commitments={commitments} salary={salary} />
              )}
            </>
          )}
          ...
          {/* <div
            style={{
              marginTop: 36, background: "#0e0e0e", border: "1px solid #141414",
              borderRadius: 12, padding: 18, fontFamily: mono,
            }}
          >
            <p style={{ ...labelStyle, marginBottom: 10 }}>Directus Setup Guide</p>
            {[
              [
                "1. commitments collection",
                "Fields: name (string), amount (decimal), months (integer), paid (integer), color (string). Enable user_created auto-field.",
              ],
              [
                "2. user_profiles collection",
                "Fields: user (M2O → directus_users), salary (decimal, nullable).",
              ],
              [
                "3. Permissions",
                "commitments: filter user_created = $CURRENT_USER for all CRUD. user_profiles: filter user = $CURRENT_USER.",
              ],
            ].map(([title, desc]) => (
              <div key={title} style={{ marginBottom: 12 }}>
                <p style={{ fontSize: 10, color: "#e76f51", marginBottom: 3 }}>
                  {title}
                </p>
